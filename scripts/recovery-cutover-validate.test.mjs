...
import { EXPECTED_TARGET, EXPECTED_TARGET_HOST, EXPECTED_SOURCE_HOST, EXPECTED_RUN, PRODUCTION_MODEL_IDS, PRODUCTION_BACKUP_PREFIX, prepareTarget, foreignKeyViolationSql, quoteIdentifier, assertKnownIncomingForeignKeys, assertProductionEvidence } from "./recovery-cutover-validate.mjs";
...
test("cleanup requires exact run and explicit cleanup approval; proof does not authorize mutation", () => {
  ...
  assert.doesNotThrow(() => prepareTarget(env, "production-proof"));
...
const production = { prediction_count: 3, matched_revision_count: 3, profile_count: 3, fixture_count: 1,
  first_generated_at_utc: "2026-09-13T15:30:00.000Z", latest_generated_at_utc: "2026-09-13T15:30:01.000Z" };
const backup = { id: "00000000-0000-0000-0000-000000000001", artifact_type: "logical_export", verification_status: "succeeded",
  storage_url: `${PRODUCTION_BACKUP_PREFIX}postgres-logical-2026-09-13T15-31-00-000Z.jsonl.gz`, sha256: "a".repeat(64), bytes: "1234",
  created_at_utc: "2026-09-13T15:31:30.000Z", verified_at_utc: "2026-09-13T15:31:31.000Z", row_counts: { predictions: 506, matches: 403 } };
test("production proof accepts first real three-profile batch with a newer verified scoped backup", () => {
  ...
  assert.doesNotThrow(() => assertProductionEvidence(production, backup));
});
test("production proof blocks missing revisions/profiles, stale data and absent or wrong backups", () => {
  for (const changes of [{ prediction_count: 2 }, { matched_revision_count: 2 }, { profile_count: 2 }, { fixture_count: 0 },
    { first_generated_at_utc: "2026-08-26T06:00:00.000Z" }, { first_generated_at_utc: "2099-01-01T12:00:00.000Z" }]) assert.throws(() => assertProductionEvidence({ ...production, ...changes }, backup));
  assert.throws(() => assertProductionEvidence(production, undefined));
  for (const changes of [{ verification_status: "failed" }, { created_at_utc: "2026-09-13T15:00:00.000Z" },
    ...
    assert.throws(() => assertProductionEvidence(production, { ...backup, ...changes }));
  }
});
