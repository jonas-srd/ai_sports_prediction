...
export default async function GermanTeamNewsPage({ params }: PageProps) {
  const { competitionSlug, teamSlug } = await params;

  redirect(`${localizePath(`/football/team/${teamSlug}/news`, "de")}?from=${encodeURIComponent(competitionSlug)}`);
}
