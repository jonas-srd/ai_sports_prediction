...
export default async function GermanTeamMatchdayPage({ params }: PageProps) {
  const { competitionSlug, teamSlug } = await params;

  redirect(`${localizePath(`/football/team/${teamSlug}/spieltag`, "de")}?from=${encodeURIComponent(competitionSlug)}`);
}
