...
export default async function TeamNewsPage({ params }: PageProps) {
  const { competitionSlug, teamSlug } = await params;

  redirect(`${localizePath(`/football/team/${teamSlug}/news`, "en")}?from=${encodeURIComponent(competitionSlug)}`);
}
